-- Switching jobs must never stop the old timer unless the new timer can start.
-- This function locks the current timer, records it, and starts the replacement
-- in one transaction after verifying job access and time-clock settings.
...
begin
  ...
  if not v_job.time_clock_enabled then
    raise exception 'Enable the time clock for this job before starting a timer.';
  end if;
